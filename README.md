...
Storage 页面将目录内容显示为二维面积图，块的面积代表实际分配的磁盘空间。点击目录块或使用方向键与回车可以逐级进入，`Backspace` 或左方向键返回上级，`Home` 回到扫描根。普通用户运行时，扫描范围限制在当前用户的 Home；使用 `sudo fleetty top` 或 root 运行时，扫描范围为 `/`。扫描不会跟随符号链接，并自动排除 NFS、SMB、SSHFS 等网络挂载以及 `/proc`、`/sys` 等虚拟文件系统。该页面只在本地 `fleetty top` 中提供，远程监控会话不会获得服务进程的文件系统扫描权限。
