...
| 按键 | 功能 |
| --- | --- |
| `1`–`4` | 打开 Overview、Compute、Network 或 Processes 页面 |
...
从 Hub 打开的计算节点使用和本地模式相同的四个专用页面。GPU、Slurm、电池、网络进程归因等能力按目标节点实际支持情况启用；不可用区域不会占位。每个连接的当前页面、主题和高级布局彼此独立，不会改变其他用户的界面：
