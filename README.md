...
按 `l` 可以打开布局编辑器。CPU、内存、磁盘、网络、电池、GPU、节点队列和进程都是独立小组件，可以调整顺序、隐藏，并选择 `SMALL`、`MEDIUM` 或 `LARGE` 尺寸。Fleetty 会根据终端宽度自动排成 1–4 列；大尺寸组件会增加指标种类和表格行，而不仅是放大图表，中尺寸组件横跨两列，小尺寸组件适合摘要浏览。例如，macOS 上的大尺寸网络组件会通过系统 `nettop` 数据展示按应用和进程划分的实时上下行速率与累计流量；其他系统在无法安全获取进程归因时展示接口级明细。平台不支持的组件会自动隐藏，例如没有电池的桌面 Mac 不显示电池组件，非 GPU 主机不显示 GPU 组件。

布局调整不需要管理权限。使用 `K` / `J` 调整顺序、`-` / `+` 调整尺寸、`Space` 隐藏或显示；鼠标也可以选择组件和点击编辑按钮。主界面内容超过终端高度时，可以使用鼠标滚轮、`PageUp` / `PageDown`、`Home` / `End` 浏览。在本地 `fleetty top` 中按 `s` 会保存到当前用户的配置目录，后续启动自动恢复；也可以通过 `--layout PATH` 或 `FLEETTY_LAYOUT_FILE` 指定布局文件。
...
每个连接都有独立的组件布局。默认情况下，`NODE QUEUE` 和 `PROCESSES` 使用大尺寸；用户可以按自己的关注点把任意指标移到前面或调整信息密度。SSH/Hub 会话中的布局调整只影响当前连接，不会改变其他用户的界面：
