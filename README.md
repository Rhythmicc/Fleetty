...
按 `l` 可以打开布局编辑器。CPU、内存、磁盘、网络、电池、GPU、节点队列和进程都是独立小组件，可以调整顺序、隐藏，并选择 `SMALL`、`MEDIUM` 或 `LARGE` 尺寸。Fleetty 会根据终端宽度自动排成 1–4 列；大尺寸组件展示更多历史或表格行，中尺寸组件横跨两列，小尺寸组件适合摘要浏览。平台不支持的组件会自动隐藏，例如没有电池的桌面 Mac 不显示电池组件，非 GPU 主机不显示 GPU 组件。
