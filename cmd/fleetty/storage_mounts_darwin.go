...
func localStorageMountPolicy() (storageMountPolicy, error) {
	...
	if err != nil {
		return storageMountPolicy{excluded: make(map[string]string)}, err
	}
	if count == 0 {
		return storageMountPolicy{excluded: make(map[string]string)}, nil
	}
	...
	if count > len(mounts) {
		return storageMountPolicy{excluded: make(map[string]string)},
