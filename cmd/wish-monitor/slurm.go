...
func collectSlurmSnapshot(config slurmClusterConfig) (slurmSnapshot, error) {
	...
	jobOutput, err := runner.Run(ctx, "squeue", "-h", "-o", "%i\t%P\t%j\t%u\t%T\t%M\t%l\t%D\t%R")
	if err != nil {
		return slurmSnapshot{}, err
	}
...
func parseSlurmJobs(output string, filter []string) ([]slurmJob, error) {
	...
	for lineNumber, line := range strings.Split(strings.TrimSpace(output), "\n") {
		...
		fields := strings.Split(line, "\t")
		if len(fields) != 9 {
			return nil, fmt.Errorf("parse squeue line %d: expected 9 fields", lineNumber+1)
		}
		...
		jobs = append(jobs, slurmJob{
			...
			TimeLimit: strings.TrimSpace(fields[6]), Nodes: nodes, Reason: strings.TrimSpace(fields[8]),
		})
	}
	return jobs, nil
}
...
func sortSlurmJobs(jobs []slurmDisplayJob) {
	...
	sort.SliceStable(jobs, func(i, j int) bool {
		left, right := rank(jobs[i].Job.State), rank(jobs[j].Job.State)
		if left != right {
			return left < right
		}
...
func (m *hubModel) slurmQueueView() string {
	...
	jobMeta := fmt.Sprintf("%s  ·  %d RUN  ·  %d PEND  ·  %d JOBS", filter, running, pending, len(jobs))
	...
	if state.Error != "" {
		...
		content = []string{
			fmt.Sprintf("%s %d   %s %d   %s %d",
				...
				processWaitingStyle.Render("PEND"), pending,
...
func (m *hubModel) selectedSlurmJobs() []slurmDisplayJob {
	...
	for index, state := range m.slurmStates {
		...
		for _, job := range state.Snapshot.Jobs {
			jobs = append(jobs, slurmDisplayJob{Cluster: state.Snapshot.Name, Job: job})
		}
	}
	sortSlurmJobs(jobs)
	return jobs
...
func renderSlurmJobRow(display slurmDisplayJob, width int) string {
	job := display.Job
	stateStyle := slurmJobStateStyle(job.State)
	reasonStyle := dimStyle
	if strings.EqualFold(job.State, "PENDING") {
	...
	case width >= 124:
		...
		return strings.Join([]string{
			...
			stateStyle.Render(fixedCell(job.State, 11, false)),
	...
	case width >= 84:
		...
		return strings.Join([]string{
			...
			stateStyle.Render(fixedCell(job.State, 10, false)),
	...
	default:
		return valueStyle.Render(fixedCell(job.ID, 13, false)) + " " +
			stateStyle.Render(fixedCell(job.State, 9, false)) + " " +
